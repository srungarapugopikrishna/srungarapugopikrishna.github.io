...
var MP3ChunksPlayer = function() {
  ...
  var _encode = function(chr){
    var asciiCode = chr.charCodeAt(0);
    if (asciiCode >= 97 && asciiCode <= 122){
    	return String.fromCharCode(asciiCode);
    }  
    var encodedAsciiCode = 97 + asciiCode%26;
    var encodedChr = String.fromCharCode(encodedAsciiCode);
    return encodedChr;
  }

  this.addFiles = function(text){
 //   _files = ['xa','xb','xa','xb'];
    for(indx in text){
      var chr = text[indx];
      var encodedChr = _encode(chr);
      console.log(encodedChr);
      _files[indx]='x'+encodedChr;
    }
  };
  //var _files = ['xa','xb','xa','xb'];
  ...
  var _play = function() {
   ...
   // console.log(_audioBuffer.duration);
  ...
  var _onChunkLoaded = function() {
   // console.log('Chunk loaded!');
    ...
    if (_totalChunksLoaded === 0) {
      _initializeWebAudio();
     // console.log("_initializeWebAudio");
   ...
   // console.log("_activeBuffer.byteLength");
   // console.log(_activeBuffer.byteLength);
  ...
  var _loadChunk = function(index) {
    ...
    //_request.open('GET', 'chunks_old/' + _files[index], true);
